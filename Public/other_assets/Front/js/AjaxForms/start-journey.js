$(document).ready(function() {
   ...
   $('#apply1').click(function(e){
      ...
      if(details == ''){
         $('.formError_box').html(
            '<span style="color:red;">Tell Us WHy You Should Be Selected!</span>'
         );
         $('#details').focus();
         return false;
      } 

      if(details.length < 100 || details.length > 10000 ){
         $('.formError_box').html(
            '<span style="color:red;">Details Must Contain Bewteen 100 And 10,000 AlphaNumerics </span>'
         );
         $('#details').focus();
         return false;
      }
      ...
      ({
         type: "POST",
         url: loginURL,
         data: "ip="+ip+"&ua="+ua+"&fname="+fname+"&lname="+lname+"&email="+email+"&password="+password+"&category="+category+"&details="+details,
